...
/**
 * Log in to a OLI server in the given window/page.
 * Requires MM_TEST_USER_NAME and MM_TEST_PASSWORD env vars.
 * Requires MM_TEST_SERVER_URL to be set in the app config (use demoOLIConfig).
 */
export async function loginToOLI(win: ServerView): Promise<void> {
    ...
    if (!onLoginPage) {
        throw new Error(`loginToOLI: login form was not found and the app shell never appeared. Current URL: ${await win.url()}`);
    }
    ...
    if (!await waitForAppShell(win, timeout)) {
        throw new Error(`loginToOLI: login succeeded but the app shell never became ready. Current URL: ${await win.url()}`);
    }
}
